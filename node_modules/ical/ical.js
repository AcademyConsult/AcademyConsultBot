  ...
  return {
    ...
    objectHandlers : {
      ...
      , 'END' : function(component, params, curr, stack){
        ...
        if (curr.uid) {
          // the same (recurring) event may be included multiple times
          // with different sequence numbers
          if (curr.sequence) {
              par[curr.uid + '-' + curr.sequence] = curr
          } else {
              par[curr.uid] = curr
          }
        } else {
    ...
    parseICS : function(str){
      ...
      for (var i = 0, ii = lines.length, l = lines[0]; i<ii; i++, l=lines[i]){
        ...
        // workaround: Office365 proviedes some time zones including diff to UTC, breaking the parser here, e.g.
        // l = 'DTSTART;TZID="(UTC+01:00) Amsterdam, Berlin, Bern, Rom, Stockholm, Wien":20160821T180000'
        // misplaced split here -----^
        l = l.replace(/^([^:]+);TZID="[^"]*"/, "$1");
        var kv = l.split(":")
